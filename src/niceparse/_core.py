"""Clean-room reimplementation of the standard library ``argparse`` API.

This module provides ``niceparse``'s own parsing engine. It does not import or
delegate to the stdlib ``argparse`` at runtime; it reimplements the public
surface and parsing semantics to match CPython 3.14's ``argparse`` faithfully,
so it is a drop-in replacement.

Color support mirrors 3.14, which relies on the private ``_colorize`` module.
When that module is unavailable (Python < 3.13), a null theme is used and all
output is uncolored — the rest of the behavior is unchanged.
"""
...
class _NullTheme:
    """Fallback theme used when ``_colorize`` is unavailable.

    Every attribute access returns an empty string, so formatted output
...
class HelpFormatter(object):
    ...
    def add_argument(self, action):
        if action.help is not SUPPRESS:
            ...
            get_invocation = lambda x: self._decolor(
    ...
    def _format_usage(self, usage, actions, groups, prefix):
        ...
        if usage is not None:
            usage = (
                t.prog_extra
                + usage % {"prog": f"{t.prog}{self._prog}{t.reset}{t.prog_extra}"}
                + t.reset
            )
        ...
        for action in actions:
            if not action.option_strings:
                ...
                if group:
                    group_actions = [
                        action2
                        for action2 in group._group_actions
                        if action2.option_strings
                        and action_groups.pop(action2, None)
                    ] + [action]
        ...
        for option_string in option_strings:
            ...
            if option_string.startswith("--"):
                if option_string.startswith("--no-"):
                    raise ValueError(
                        f"invalid option name {option_string!r} "
...
class _SubParsersAction(Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        try:
            ...
            msg = (
                _("unknown parser %(parser_name)r (choices: %(choices)s)") % args
            )
...
class _ActionsContainer(object):
    ...
    def add_argument(self, *args, **kwargs):
        ...
        if type_func is FileType:
            raise TypeError(
                f"{type_func!r} is a FileType class object, "
...
class ArgumentParser(_AttributeHolder, _ActionsContainer):
    ...
    def _parse_known_args(self, arg_strings, namespace, intermixed):
        ...
        if not intermixed:
            ...
            arg_strings = [
                s for s, c in zip(extras, extras_pattern) if c != "O"
            ]
        ...
        # separate
            ...
            for option_string in self._option_string_actions:
                if option_string == short_option_prefix:
                    ...
                    result.append(tup)
                elif self.allow_abbrev and option_string.startswith(
                    option_prefix
                ):
                    action = self._option_string_actions[option_string]
                    tup = action, option_string, sep, explicit_arg
    ...
    def _check_value(self, action, value):
        ...
        if value not in choices:
            args = {
                "value": str(value),
                "choices": ", ".join(
                    repr(str(choice)) for choice in action.choices
                ),
            }
